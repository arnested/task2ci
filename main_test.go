...
func TestRenderTemplateSubstitutesPlaceholder(t *testing.T) {
	...
	// The header should also point at the source template and tell editors
	// where to make changes — those are the AI-tool-friendly lines.
	for _, want := range []string{
		"# Source: .task2ci/workflows/ci.yaml",
		"# To change CI: edit the source template",
	} {
		if !strings.Contains(s, want) {
			t.Errorf("expected header line %q, got:\n%s", want, s)
		}
	}
...
func TestRunHelpFlagPrintsLayoutAndAINotes(t *testing.T) {
	chdir(t, t.TempDir())
	_, stderr, err := runCapture(t, "-h")
	if err != nil {
		t.Fatalf("-h should not return an error (Usage is the entire effect): %v", err)
	}
	for _, want := range []string{
		"task2ci generates GitHub Actions workflows",
		"Layout:",
		".task2ci/workflows/<name>.yaml",
		".github/workflows/<name>.yaml   generated",
		"For AI tools",
